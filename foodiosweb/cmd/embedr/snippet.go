...
func (srv *Server) postEmbedHTML(postView *appbsky.FeedDefs_PostView) (string, error) {
	...
	data := struct {
		...
		ProfileURL:    template.URL(fmt.Sprintf("https://mysky.local.social/profile/%s?ref_src=embed", aturi.Authority())),
		PostURL:       template.URL(fmt.Sprintf("https://mysky.local.social/profile/%s/post/%s?ref_src=embed", aturi.Authority(), aturi.RecordKey())),
		WidgetURL:     template.URL("https://embed.mysky.local.app/static/embed.js"),
	}
