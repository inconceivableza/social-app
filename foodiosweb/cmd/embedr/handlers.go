...
func (srv *Server) parseBlueskyURL(ctx context.Context, raw string) (*syntax.ATURI, error) {
	...
	// then try mysky.local.app post URL
	u, err := url.Parse(raw)
	if err != nil {
		return nil, err
	}
	if u.Hostname() != "mysky.local.app" {
		return nil, fmt.Errorf("only mysky.local.app URLs currently supported")
	}
	...
	if len(pathParts) != 5 || pathParts[1] != "profile" || pathParts[3] != "post" {
		return nil, fmt.Errorf("only mysky.local.app post URLs currently supported")
	}
...
func (srv *Server) WebOEmbed(c echo.Context) error {
	...
	if err != nil {
		return c.String(http.StatusBadRequest, fmt.Sprintf("Expected 'url' to be mysky.local.app URL or AT-URI: %v", err))
	}
	...
	data := OEmbedResponse{
		...
		AuthorURL:    fmt.Sprintf("https://mysky.local.social/profile/%s", post.Author.Handle),
		ProviderName: "Opensky Social",
		ProviderURL:  "https://mysky.local.social",
		CacheAge:     86400,
		Width:        &width,
		Height:       nil,
		HTML:         html,
