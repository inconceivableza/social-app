...
const DEFAULT_POST = 'https://mysky.local.social/profile/emilyliu.me/post/3jzn6g7ixgq2y'
const DEFAULT_URI =
...
function LandingPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        if (uri) {
          if (uri.startsWith('at://')) {
            ...
            try {
              ...
              if (!urlp.hostname.endsWith('mysky.local.app')) {
              ...
              console.log(err)
              throw new Error('Invalid Opensky URL')
            }
          }
        }
        ...
        console.error(err)
        setError(err instanceof Error ? err.message : 'Invalid Opensky URL')
      } finally {
        setLoading(false)
      }
    })()
  ...
  return (
    <main className="w-full min-h-screen flex flex-col items-center gap-8 py-14 px-4 md:pt-32 dark:bg-dimmedBgDarken dark:text-slate-200">
      <Link
        href="https://pds.mysky.local.social/about"
        className="transition-transform hover:scale-110">
        <img src={logo} className="h-10" />
      </Link>

      <h1 className="text-4xl font-bold text-center">Embed a Opensky Post</h1>
...
function toShareUrl(path: string) {
  return `https://mysky.local.social${path}?ref_src=embed`
}
