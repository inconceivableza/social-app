...
export const BSKY_APP_HOST = 'https://foodios.app'
...
export function toShareUrl(url: string): string {
  if (!url.startsWith('https')) {
    const urlp = new URL('https://foodios.app')
    urlp.pathname = url
    url = urlp.toString()
  }
  return url
...
export function isBskyAppUrl(url: string): boolean {
  return url.startsWith('https://foodios.app/')
}
...
export function isBskyRSSUrl(url: string): boolean {
  return (
    (url.startsWith('https://foodios.app/') || isRelativeUrl(url)) &&
    /\/rss\/?$/.test(url)
  )
}
...
export function createProxiedUrl(url: string): string {
  ...
  return `https://go.foodios.app/redirect?u=${encodeURIComponent(url)}`
}

export function isShortLink(url: string): boolean {
  return url.startsWith('https://go.foodios.app/')
}
