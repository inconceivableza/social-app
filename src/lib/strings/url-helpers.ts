...
export const BSKY_APP_HOST = 'https://mysky.local.social'
...
export function toShareUrl(url: string): string {
  if (!url.startsWith('https')) {
    const urlp = new URL('https://mysky.local.social')
    urlp.pathname = url
    url = urlp.toString()
  }
  return url
...
export function isBskyAppUrl(url: string): boolean {
  return url.startsWith('https://mysky.local.social/')
}
...
export function isBskyRSSUrl(url: string): boolean {
  return (
    (url.startsWith('https://mysky.local.social/') || isRelativeUrl(url)) &&
    /\/rss\/?$/.test(url)
  )
}
...
export function createProxiedUrl(url: string): string {
  ...
  return `https://go.mysky.local.app/redirect?u=${encodeURIComponent(url)}`
}

export function isShortLink(url: string): boolean {
  return url.startsWith('https://go.mysky.local.app/')
}
