...
function initPreparationState(record: AppFoodiosFeedRecipeRevision.Record): PreparationState {
    return {
        ingredients: record.ingredients.map(() => ({ checked: false })),
        instructionSections: record.instructionSections.map(({ instructions }) => instructions.map(() => ({ checked: false })))
    }
}

export function usePreparationState(revision: AppFoodiosFeedDefs.RecipeRevisionView) {
    const { currentAccount } = useSession()
    const [storedState, setStoredState] = useStorage(account,
        [currentAccount?.did ?? 'pwi', "recipePreparations"])
    ...
    const dispatch = useCallback((action: PreparationAction) => {
        // Possibility of out of order updates here
        const updatedState = {
            ...storedState, [revision.selectedRevisionUri]:
                preparationReducer(preparationState, action)
        }
        setStoredState(updatedState)
    }, [preparationState, storedState, revision.selectedRevisionUri])
