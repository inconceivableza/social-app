...
describe('toNiceDomain', () => {
  const inputs = [
    ...
    'https://mysky.local.social',
    'https://pds.mysky.local.social',
    '#123123123',
  ]
  const outputs = ['example.com', 'mysky.local.app', 'Opensky Social', '#123123123']
...
describe('toShortUrl', () => {
  const inputs = [
    'https://mysky.local.social',
    'https://mysky.local.social/3jk7x4irgv52r',
    'https://mysky.local.social/3jk7x4irgv52r2313y182h9',
    'https://very-long-domain-name.com/foo',
    'https://very-long-domain-name.com/foo?bar=baz#andsomemore',
  ]
  const outputs = [
    'mysky.local.app',
    'mysky.local.app/3jk7x4irgv52r',
    'mysky.local.app/3jk7x4irgv52...',
...
describe('toShareUrl', () => {
  const inputs = ['https://mysky.local.social', '/3jk7x4irgv52r', 'item/test/123']
  const outputs = [
    'https://mysky.local.social',
    'https://mysky.local.social/3jk7x4irgv52r',
    'https://mysky.local.social/item/test/123',
  ]
  ...
  const outputs = [
    {
      type: 'youtube_video',
      source: 'youtube',
      playerUri: 'https://mysky.local.social/iframe/youtube.html?videoId=videoId&start=0',
    },
    {
      type: 'youtube_video',
      source: 'youtube',
      playerUri: 'https://mysky.local.social/iframe/youtube.html?videoId=videoId&start=1',
    },
    {
      type: 'youtube_video',
      source: 'youtube',
...

    {
      ...
      isGif: true,
      hideDetails: true,
      playerUri: 'https://t.gifs.mysky.local.app/someID_AAAAM/someName.gif',
      dimensions: {
        width: 100,
        height: 100,
      },
...
describe('parseStarterPackHttpUri', () => {
  const baseUri = 'https://mysky.local.social/start'
  ...
  it('returns null when the route is not /start or /starter-pack', () => {
    const validHttpUri = 'https://mysky.local.social/start/haileyok.com/rkey'
    expect(parseStarterPackUri(validHttpUri)).toEqual({
      name: 'haileyok.com',
      rkey: 'rkey',
    })

    const validHttpUri2 = 'https://mysky.local.social/starter-pack/haileyok.com/rkey'
    expect(parseStarterPackUri(validHttpUri2)).toEqual({
      name: 'haileyok.com',
      rkey: 'rkey',
    })

    const invalidHttpUri = 'https://mysky.local.social/profile/haileyok.com/rkey'
...
describe('createStarterPackGooglePlayUri', () => {
  const base =
    'https://play.google.com/store/apps/details?id=xyz.mysky.app&referrer=utm_source%3Dbluesky%26utm_medium%3Dstarterpack%26utm_content%3Dstarterpack_'
...
describe('tenorUrlToBskyGifUrl', () => {
  ...
  it.each(inputs)(
    'returns url with t.gifs.mysky.local.app as hostname for input url',
    input => {
      const out = tenorUrlToBskyGifUrl(input)
      expect(out.startsWith('https://t.gifs.mysky.local.app/')).toEqual(true)
    },
  )
})
