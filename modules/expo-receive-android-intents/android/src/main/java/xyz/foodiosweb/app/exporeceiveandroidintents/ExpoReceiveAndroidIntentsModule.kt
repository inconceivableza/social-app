package xyz.foodios.app.exporeceiveandroidintents
...
class ExpoReceiveAndroidIntentsModule : Module() {
  ...
  private fun handleTextIntent(intent: Intent) {
    intent.getStringExtra(Intent.EXTRA_TEXT)?.let {
      ...
      "foodios://intent/compose?text=$encoded".toUri().let { uri ->
  ...
  private fun handleImageIntents(uris: List<Uri>) {
    ...
    "foodios://intent/compose?imageUris=$encoded".toUri().let {
  ...
  private fun handleVideoIntents(uris: List<Uri>) {
    ...
    "foodios://intent/compose?videoUri=${URLEncoder.encode(file.path, "UTF-8")}|${info["width"]}|${info["height"]}".toUri().let {
      val newIntent = Intent(Intent.ACTION_VIEW, it)
      appContext.currentActivity?.startActivity(newIntent)
    }
  }
