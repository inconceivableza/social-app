...
func (srv *Server) postEmbedHTML(postView *appbsky.FeedDefs_PostView) (string, error) {
	...
	data := struct {
		...
		ProfileURL:    template.URL(fmt.Sprintf("https://foodios.app/profile/%s?ref_src=embed", aturi.Authority())),
		PostURL:       template.URL(fmt.Sprintf("https://foodios.app/profile/%s/post/%s?ref_src=embed", aturi.Authority(), aturi.RecordKey())),
		WidgetURL:     template.URL("https://embed.foodios.app/static/embed.js"),
	}
